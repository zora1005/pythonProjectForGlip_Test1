...
def invitePeople(tk, portID, creator_id,email,messageCount,type):
    baseUrl = getGlipBaseUrl(portID) +"/api/person"
    paras = {'invite_id': creator_id, 'email': email, 'tk': tk}
    payload=json.dumps(paras)
    invitePeopleResponse = requests.post(baseUrl,data=payload, headers=header)
    #print(createTeamResonse.json())
    #print(createTeamResonse.status_code)19701766


    if invitePeopleResponse.ok:
        print('Invited People: ')
        print(email)
        if type == 'post' or type == 'Post':
            groupID = invitePeopleRespone.json()._id #传一个字符串对象
            for index in range(0, messageCount):
                replyPost(tk,portID, creator_id,groupID,'repiedtestToday_' + str(index))
...
def run(ENV,userName,extNum,passWord,portID, createTeamCount,messageCount,type):
    ...
    for index in range(0, createTeamCount):
        invitePeople(glip_token['tk'], portID, glip_token['creator_id'], emailpre + str(index) + "@acmec2.com",messageCount,type)
        #createTeam(glip_token['tk'],portID, glip_token['creator_id'], teamName + str(index), members, teamName + str(index),messageCount,type)
        time.sleep(0.15)


run('http://api-up.lab.rcch.ringcentral.com','18002491122','106','Test!123','23304',2,2, 'post')
